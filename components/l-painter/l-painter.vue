...
<script>
import Pen, { penCache, clearPenCache } from './static/pen';
import Downloader from './static/downloader';
...
export default {
    ...
    methods: {
        ...
        initDancePalette() {
            ...
            this.initScreenK();
            this.downloadImages(this.dancePalette).then(async (palette) => {
				...
				this.painterStyle = `width:${width.toPx()}px;height:${height.toPx()}px;`
                this.frontContext || (this.frontContext = await this.getCanvasContext(this.use2D, 'front'));
                this.bottomContext || (this.bottomContext = await this.getCanvasContext(this.use2D, 'bottom'));
                this.topContext || (this.topContext = await this.getCanvasContext(this.use2D, 'top'));
                this.globalContext || (this.globalContext = await this.getCanvasContext(this.use2D, 'k-canvas'));
                new Pen(this.bottomContext, palette, this.use2D).paint(() => {
        ...
        startPaint() {
            this.initScreenK();
			const dpr = uni.getSystemInfoSync().devicePixelRatio.toFixed(1)
            ...
            if (width.toPx() !== this.canvasWidthInPx) {
                this.canvasWidthInPx = width.toPx()*dpr;
                needScale = this.use2D;
            }
            if (this.widthPixels) {
            ...
            if (this.canvasHeightInPx !== height.toPx()) {
                this.canvasHeightInPx = height.toPx()*dpr;
                needScale = needScale || this.use2D;
            }
			...
			this.downloadImages(this.palette).then(async (palette) => {
			    if (!this.photoContext) {
			        this.photoContext = await this.getCanvasContext(this.use2D, 'photo');
			    }
			    if (needScale) {
			        const scale = getApp().globalData.systemInfo.pixelRatio;
        ...
        saveImgToLocal() {
            const that = this;
			console.log(that.canvasWidthInPx);
            setTimeout(() => {
                uni.canvasToTempFilePath(
                    {
                        canvasId: 'photo',
                        canvas: that.use2D ? that.canvasNode : null,
    ...
    watch: {
        ...
        dancePalette: {
            handler: function (newVal, oldVal) {
                if (!this.isEmpty(newVal) && !this.use2D) {
                    clearPenCache();
        ...
        action: {
            handler: function (newVal, oldVal) {
                if (newVal && !this.isEmpty(newVal) && !this.use2D) {
