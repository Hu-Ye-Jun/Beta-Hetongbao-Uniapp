...
function multiplyWithUnit(str, factor) {
	console.log(str);
  const match = str.match(/^(\d+)(rpx|px)$/);  
  if (!match) {
	throw new Error('单位格式无效');  
  }
...
export default class Painter {
    constructor(ctx, data) {
		this.dpr = uni.getSystemInfoSync().devicePixelRatio.toFixed(1)
        this.ctx = ctx;
		this.height = multiplyWithUnit(data.height,this.dpr)
		this.width = multiplyWithUnit(data.width,this.dpr)
		this.views = data.views.map(e=>{
			e.css.top = multiplyWithUnit(e.css.top,this.dpr)
			e.css.left = multiplyWithUnit(e.css.left,this.dpr)
			e.css.width = multiplyWithUnit(e.css.width,this.dpr)
			e.css.height = multiplyWithUnit(e.css.height,this.dpr)
			if(e.css.fontSize){
				e.css.fontSize = multiplyWithUnit(e.css.fontSize,this.dpr)
			}
			return e
		})
    ...
    paint(callback) {
        this.style = {
            width: this.width.toPx()*2,
            height: this.height.toPx()*2
        };
